...
let flow = {
	...
	getLocalList: flowAction.SUCCESS,
	postFixtures: flowAction.SUCCESS,
};
...
app.post("/backoffice/customers/", upload.any(), (req, res) => {
	switch (flow.postCustomer) {
		case flowAction.SUCCESS:
			console.log(
				"company :" +
					JSON.stringify(JSON.parse(req.body.company), null, 2)
			);
			console.log(
				...
				req.body.brand_name !== ""
					? brandMock.Brand_Search_Success.filter((el) => {
							return el.name
								.toLocaleLowerCase()
								.includes(
									req.body.brand_name.toLocaleLowerCase()
								);
					  })
					: undefined;
...
// CLIENT FIXTURES GET, ADD, DEL, UPDATE//

app.get("/backoffice/fixtures/locals/", (req, res) => {
	switch (flow.getLocalList) {
		case flowAction.SUCCESS:
			res.status(200);
			res.json(localList.data_local);
			break;
		default:
			break;
	}
